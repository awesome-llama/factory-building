...
# generic 2D point or vector
struct XY {
    x,
    y
}

%define VEC2(X,Y) XY { x:(X), y:(Y) }

# generic 3D point or vector
struct XYZ {
    x,
    y,
    z
...
%define VEC3(X,Y,Z) XYZ { x:(X), y:(Y), z:(Z) }


enum CameraFollowMode {
    FREE = "FREE", # fly anywhere, controlled by user
    PLAYER = "PLAYER", # follow player object
    CUTSCENE = "CUTSCENE"
}
...
# Vector angle in the XY plane. Note the addition of 0, used to fix a negative zero bug: https://scratch.mit.edu/projects/1090711087/
